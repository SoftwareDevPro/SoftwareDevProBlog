---
title: "Sylvester Port - Qunit toUnitVector"
date: 2020-09-29T13:29:04-07:00
draft: true
---
In the last segment we added one additional test suite, for the to3D method does, which returned a new vector in 3-d space if it was previously in 2-d space, the same vector if it was already in 3-d space,  otherwise it returned null.  In this segment, we add a test suite for the toUnitVector which returned a new Vector, by normalizing the current one.
...
Details:
The toUnitVector method starts off with an initial vector, then creates several intermediate vectors, and then finally a unit vector via the toUnitVector method.  At the end of the test suite, we verify the values on each of the vector elements.
