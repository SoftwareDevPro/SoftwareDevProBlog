---
title: "Sylvester Port - Qunit, zero (asynchronous)"
date: 2020-10-17T09:16:11-07:00
draft: true
---
In the previous segment, we added in the modified Random test method call.  In this segment, we add in the Zero test method, and modify it accordingly.  Recall that the Zero method, is also an class  method (one that can be invoked without an instance of the class) that returns a Vector with N 0 elements. As with all the previous test method modifications, change the test call to an asyncTest call, and add the invocation of the start method.
...
asyncTest("vector.Random", function()  {
  expect(6)
  v1 = Vector.Random(5);
...
asyncTest("vector.Zero", function()  {
  expect(6);
  v1 = Vector.Zero(5);
  equal(5, v1.dimensions());
  equal(0, v1.e(1));
