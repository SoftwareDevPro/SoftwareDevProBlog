---
title: "Sylvester Port - Qunit, max"
date: 2020-09-28T09:44:39-07:00
draft: true
---

In the last segment we added one additional test suite, for the snapTo method which returns the first index into the vector for the item desired.  In this segment, we add a test suite for the max method, which returns the maximum value for the vector.
...
Details:
The max method test suite, creates one vector, and then sets the elements on it, and checks the max value with the ok test method, and then we set the elements on the same vector, and verify the max value again with the ok test method.
