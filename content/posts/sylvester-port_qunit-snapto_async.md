---
title: "Sylvester Port - Qunit, snapTo (asynchronous)"
date: 2020-10-13T10:20:08-07:00
draft: true
---

In this segment, we add in the snapTo method.  In the last segment, we added the round method which rounds each element of the vector, and returns the result.  The snapTo method returns a copy of the vector with the elements set to the given value if they differ from it less than the Sylvester precision value (precision: 1e-6).  Once again we change the test method to an asyncTest method, and add an invocation of the start method to the end of the test method.
