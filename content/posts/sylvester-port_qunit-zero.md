---
title: "Sylvester Port - Qunit, zero"
date: 2020-09-30T15:32:05-07:00
draft: true
---
In the last segment we added one additional test suite, for the Random class method creates a new Vector, and returns it with the specified number of dimensions, each with random values.  In this last segment for the unit testing of the Vector class, we add a test suite for the Zero class method, which creates a n-dimensioned  Vector with all zero values, and returns it to the caller.
...
Details:
The Zero class method test suite, creates a five dimensional based vector, and then the returned vector is checked for the correct dimensions and values (all zero).
