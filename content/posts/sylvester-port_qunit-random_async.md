---
title: "Sylvester Port - Qunit, random (asynchronous)"
date: 2020-10-16T17:49:22-07:00
draft: true
---
In the previous segment, we added in the modified toUnitVector test method call.  In this segment, we add in the Random test method, and modify it accordingly.  Recall that the Random method, is an class  method (one that can be invoked without an instance of the class) that returns a Vector with N random elements. As with all the previous test method modifications, change the test call to an asyncTest call, and add the invocation of the start method.
