---
title: "Sylvester Port - Qunit, modulus"
date: 2020-09-28T09:44:43-07:00
draft: true
---

In the last segment we added one additional test suite, for the max method, which returns the maximum value for the vector.  In this segment we add a test suite, for the modulus method, which doesn't operate anything like its normal math operator %, here the modulus method, returns the length of the vector.
...
Details:
Similar to some of the other test suites, we create an initial vector, and then a bunch of intermediate vectors, and then finally, we check the modulus value on the v9 vector.
